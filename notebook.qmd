...
$$\Delta^{RM}(\bar M) = \{ \delta : \forall t \geq0, | \delta_{t+1}-\delta_t| \leq \bar M * max_{s <0}|\delta_{s+1}-\delta_s| \}$$
    ...
    - Authors highlight its relevance in cases where one wants to test whether post-period estimates might be caused by confounding *shocks*, of magnitude comparable to the highest pre-intervention confounding shock^[Par défaut les fonctions implémentées dans R choisissent la valeur de $|\delta_s|$ la plus élevée dans la période pre-treatment. Rien n'empêche en théorie de choisir les périodes pre- qu'on juge les plus pertinentes.].  
